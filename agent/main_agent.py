...
class AgentState(TypedDict):
    """
    Shared state dictionary passed between all LangGraph nodes in the pipeline.
    ...
    Attributes:
        ...
        chart_attributes: Parsed JSON dict describing chart type, axes, and title.
        data:             JSON-serialised query result (records + chart_attributes).
        chart_json:       Final chart output — either a Plotly JSON string or dashboard XML.
        insights:         LLM-generated business insights bullet points.
        error:            Last SQL execution error message (empty string if none).
        retry_count:      Number of SQL generation retries attempted so far.
    """
    question:str
...
async def build_and_run_pipeline(question: str) -> AgentState:
    """
    Build and execute the single-chart analytics LangGraph pipeline.

    Defines all pipeline nodes inline, wires them into a StateGraph, compiles
    it, and invokes it asynchronously.  The pipeline follows this sequence:

        retrieve_context → get_schema → generate_sql → decide_chart_attributes
            → execute_sql → [retry or generate_chart] → generate_insights

    SQL execution errors trigger up to 3 automatic retries via a conditional
    edge back to ``generate_sql``; after that, ``handle_sql_error`` is called.
    ...
    Returns:
        ...
        ``sql_query``, and all intermediate fields.
    """
...
async def run_agent():
    """
    Interactive CLI entry point for the single-chart analytics agent.

    Runs an async read-eval-print loop that accepts natural-language questions,
    passes them to ``build_and_run_pipeline``, prints the generated insights,
    and saves any dashboard XML to ``chart_output.xml``.  Type ``quit`` to exit.
    """
